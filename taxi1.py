...
#train = pd.read_csv('D:/Py datasets/korolev2/taxi/train.csv')
#test = pd.read_csv('D:/Py datasets/korolev2/taxi/test.csv')
# sample_submission = pd.read_csv('D:/Py datasets/korolev2/taxi/sample_submission.csv')

train = pd.read_csv('C:/Users/avarchenko.FORMAG/Documents/Py_datasets/taxi/train.csv')
test = pd.read_csv('C:/Users/avarchenko.FORMAG/Documents/Py_datasets/taxi/test.csv')
# sample_submission = pd.read_csv('C:/Users/avarchenko.FORMAG/Documents/Py_datasets/taxi/sample_submission.csv')
...
def get_dd_range(xx):
    if xx >=19:
        ...
        return 5
    elif xx >= 10 and xx < 17:
        return 6
    elif xx >= 17 and xx < 18:
        return 7
    else:
        return 8

        
# bbb = [train['pickup_datetime'].head(), train['pickup_datetime'].dt.hour.head(), train['pickup_datetime'].dt.hour.head().apply(get_dd_range)]
# a2 = train[['vendor_id', 'pickup_datetime']].head()
# a2.loc[:, 'ddd'] = a2['pickup_datetime'].dt.hour.head()
# a2.loc[:, 'h_range'] = a2['pickup_datetime'].dt.hour.head().apply(get_dd_range)
# a3 = pd.get_dummies(a2['h_range'], prefix = 'hour_range')
# a2 = a2.join(a3)
        
train.loc[:, 'h_range'] = train['pickup_hour'].apply(get_dd_range)
train = train.join( pd.get_dummies(train['h_range'], prefix = 'hour_range')  )
train = train.drop('h_range',1)

test.loc[:, 'h_range'] = test['pickup_hour'].apply(get_dd_range)
test = test.join( pd.get_dummies(test['h_range'], prefix = 'hour_range')  )
test = test.drop('h_range',1)
...
# OSRM Features
fr1 = pd.read_csv('C:/Users/avarchenko.FORMAG/Documents/Py_datasets/taxi/osrm/fastest_routes_train_part_1.csv', usecols=['id', 'total_distance', 'total_travel_time', 'number_of_steps'])
fr2 = pd.read_csv('C:/Users/avarchenko.FORMAG/Documents/Py_datasets/taxi/osrm/fastest_routes_train_part_2.csv', usecols=['id', 'total_distance', 'total_travel_time', 'number_of_steps'])
test_street_info = pd.read_csv('C:/Users/avarchenko.FORMAG/Documents/Py_datasets/taxi/osrm/fastest_routes_test.csv',
...
# Modeling
Xtr, Xv, ytr, yv = train_test_split(train[feature_names].values, y, test_size=0.2, random_state=27947)
...
#vlambda = [0, 0.001, 0.003, 0.01, 0.03, 0.1, 0.3, 1, 3, 10, 15, 20, 30]
#vlambda = [15, 20, 30, 50, 100]
vlambda = [0]
for ilambda in vlambda:

    # Try different parameters! My favorite is random search :)
    xgb_pars = {'min_child_weight': 50, 'eta': 0.3, 'colsample_bytree': 0.3, 'max_depth': 10,
                'subsample': 0.8, 'lambda': ilambda, 'nthread': -1, 'booster' : 'gbtree', 'silent': 1,
                'eval_metric': 'rmse', 'objective': 'reg:linear'}
    
 
    #cv_output = xgb.cv(xgb_pars, dtrain, num_boost_round=1000, early_stopping_rounds=20,verbose_eval=10, show_stdv=False)
    #cv_output[['train-rmse-mean', 'test-rmse-mean']].plot()

    
    # You could try to train with more epoch
    model = xgb.train(xgb_pars, dtrain, 50, watchlist, early_stopping_rounds=50, maximize=False, verbose_eval=10)
    
    
    print('\x1b[0;31;46m' + 'Modeling RMSLE %.5f' % model.best_score + '\x1b[0m')
    print('\x1b[0;31;46m' + 'Lambda %.5f' % ilambda + '\x1b[0m')
    t1 = dt.datetime.now()
...
rmse_wo_feature = [0.39224, 0.38816, 0.38726, 0.38780, 0.38773, 0.38792, 0.38753, 0.38745, 0.38710, 0.38767, 0.38738, 0.38750, 0.38678, 0.39359, 0.38672, 0.38794, 0.38694, 0.38750, 0.38742, 0.38673, 0.38754, 0.38705, 0.38736
                   , 0.38741, 0.38764, 0.38730, 0.38676, 0.38696, 0.38750, 0.38705, 0.38746, 0.38727, 0.38750, 0.38771, 0.38747, 0.38907, 0.38719, 0.38756, 0.38701, 0.38734, 0.38782, 0.38673, 0.38797, 0.38720, 0.38709, 0.38704, 0.38809, 0.38768, 0.38798, 0.38849, 0.38690, 0.38753, 0.38721, 0.38807, 0.38830, 0.38750, np.nan, np.nan, np.nan, np.nan, np.nan, np.nan, np.nan, np.nan, np.nan, np.nan]
...
with sns.axes_style("whitegrid"):
    ...
    ax.set_ylim(np.min(feature_importance['rmse_wo_feature']) - 0.001, np.max(feature_importance['rmse_wo_feature']) + 0.001)
    plt.show()
...
test[['id', 'trip_duration']].to_csv('avarch_taxi_xgb_submission.csv.gz', index=False, compression='gzip')
